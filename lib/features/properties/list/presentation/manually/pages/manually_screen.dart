...
class ManualEntryConnectionWithPropertiesScreen extends StatefulWidget {
  const ManualEntryConnectionWithPropertiesScreen({super.key});

  @override
  State<ManualEntryConnectionWithPropertiesScreen> createState() =>
      _ManualEntryConnectionWithPropertiesScreenState();
}

class _ManualEntryConnectionWithPropertiesScreenState
    extends State<ManualEntryConnectionWithPropertiesScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body:
          BlocConsumer<
            ManuallyConnectionWithPropertiesBloc,
            ManuallyConnectionWithPropertiesState
          >(
            listener: (context, state) {
              if (state is ManuallyConnectionWithPropertiesLoaded) {
                debugPrint('ManuallyLoaded state: ${state.connection}');
                context.push(
                  '/update-form',
                  extra: {'connection': state.connection, 'mode': 'manual'},
                );
              } else if (state is ManuallyConnectionWithPropertiesError) {
            ...
            builder: (context, state) {
              final isLoading =
                  state is ManuallyConnectionWithPropertiesLoading;
                            ...
                            children: [
                              ...
                              TextFormField(
                                ...
                                decoration: InputDecoration(
                                  labelText:
                                      'Acometida ID (ej: 1-256 o 12-256)',
                                ...
                                onFieldSubmitted: (_) {
                                  if (!isLoading &&
                                    ...
                                    context
                                        ...
                                        >()
                                        .add(
                                          FetchManuallyConnectionWithPropertiesEvent(
                                            acometidaId,
                                          ),
                                        );
                                  }
